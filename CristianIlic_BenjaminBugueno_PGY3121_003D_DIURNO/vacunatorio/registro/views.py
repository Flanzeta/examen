...
def ingreso(request):
    ...
    if len(nombre)>2 and len(appaterno)>2 and len(apmaterno)>2 and len(rut)>2 and len(edad)>2 and len(vacuna)>2 and len(fecha)>2 and len(nombrevacunatorio)>2 and len(direccion)>2 and len(tipoestablecimiento)>2 and len(telefono)>2 and len(email)>2:
...
def eliminarest(request):
    id_recibido = request.GET["direccion"]
    Est= Establecimiento.objects.filter(direccion=id_recibido)
    if Est:
            pro2=Establecimiento.objects.get(direccion=id_recibido)
            pro2.delete()
